...
function renderEqs(){
  ...
  var h="<tr>"+cols.map(function(c){return "<th>"+c[1]+"</th>";}).join("")+"</tr>";
  var eqs=EQS.filter(function(e){ return !FILT.mechanism||e.mechanism===FILT.mechanism; });
  ...
  if(!eqs.length) h+="<tr><td colspan='"+cols.length+"' class='small'>No equations registered for this mechanism.</td></tr>";
...
function renderVars(){
  ...
  var h="<tr>"+cols.map(function(c){return "<th>"+c[1]+"</th>";}).join("")+"</tr>";
  var vars=VARS.filter(function(v){ return !FILT.mechanism||v.mechanism===FILT.mechanism||v.mechanism==="all"; });
...
function renderDoseResponse(){
  ...
  // restrict to the selected mechanism, and narrow the Response dropdown to what that mechanism offers
  var pool=DRM.filter(function(m){ return !FILT.mechanism||m.mechanism===FILT.mechanism; });
  var drs=document.getElementById("drresp"),avail=uniqSorted(pool.map(function(m){return m.response;})),cur=drs.value;
  drs.innerHTML='<option value="all">all responses</option>'+avail.map(function(r){return '<option value="'+esc(r)+'">'+esc(r)+'</option>';}).join("");
  drs.value=(cur!=="all"&&avail.indexOf(cur)>=0)?cur:"all";
  var sel=drs.value;
  var models=pool.filter(function(m){ return sel==="all"||m.response===sel; });
...
function render(){
  ...
  renderPlot(rows); renderTable(rows); renderEqs(); renderVars(); renderCoverage(rows); renderMiss(rows); renderDoseResponse();
}
